...
preg_match_all('/^postgres:\/\/(?<user>[^:]*):(?<password>[^@]*)@(?<host>[^:]*):(?<port>[^\/]*)\/(?<database>[^$]*)$/', $ivory_url, $matches);
	...
	$conf['servers'][0]['host'] = $matches['host'][0];

	// Database port on server (5432 is the PostgreSQL default)
	$conf['servers'][0]['port'] = $matches['port'][0];
	...
	$conf['servers'][0]['defaultdb'] = $matches['database'][0];
